...
def test_expert_parallel_versus_weight_parallel(group, batch_size,
                                                sequence_length, hidden_size,
                                                ffn_hidden_size, num_experts,
                                                top_k, memory_optimized):
    init_fn = functools.partial(torch.nn.init.normal_, mean=0.0, std=0.1)
    ep_args = arguments.Arguments(hidden_size=hidden_size,
    ...
    wp_args = arguments.Arguments(hidden_size=hidden_size,
                                  ffn_hidden_size=ffn_hidden_size,
                                  moe_num_experts=num_experts,
                                  moe_top_k=top_k,
    ...
    for i in range(torch.distributed.get_world_size(group)):
        ...
        if i == rank:
            np.testing.assert_allclose(out.detach().float().cpu(),
    ...
    if rank == 0:
        np.testing.assert_allclose(wp_w2_grad.float().cpu(),
    ...
    if rank == 0:
        np.testing.assert_allclose(wp_w1_grad.float().cpu(),
